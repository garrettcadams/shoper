...
$product_images_classes = apply_filters( 'ciyashop_product_images_classes', array(
	'ciyashop-product-images',
) );


$product_images_wrapper_classes = apply_filters( 'ciyashop_product_images_wrapper_classes', array(
...
$wrapper_classes   = apply_filters( 'woocommerce_single_product_image_gallery_classes', array(
	...
	'ciyashop-product-gallery--' . $placeholder,
) );

$gallery_wrapper_classes   = apply_filters( 'ciyashop_product_image_gallery_wrapper_classes', array(
...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $product_images_classes ) ) ); ?>">
	...
	<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $product_images_wrapper_classes ) ) ); ?>">
				...
				if ( !empty($product_gallery_image_ids) ) {
					...
					foreach ( $product_gallery_image_ids as $attachment_id ) {
						...
						$attributes      = array(
							...
							'class'                   => "attachment-$size_class size-$size_class" . ( ($gallery_images_sr == 1) ?  "wp-post-image" : "" ),
						);
					...
					$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_attr__( 'Awaiting product image', 'ciyashop' ) );
					$html .= '</div>';
				}
				?>
			</div>
		...
		if ( $style != 'wide_gallery' && !empty($product_gallery_image_ids) ) {
			?>
			<div class="ciyashop-product-thumbnails">
				<div class="ciyashop-product-thumbnails__wrapper">
					...
					foreach ( $product_gallery_image_ids as $attachment_id ) {
						...
						echo apply_filters( 'ciyashop_single_product_image_thumbnail_html', $html, $attachment_id );
					}
					?>
				</div>
			</div>
