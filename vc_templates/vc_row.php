 ...
 */
$el_class = $full_height = $parallax_speed_bg = $parallax_speed_video = $full_width = $equal_height = $flex_row = $columns_placement = $content_placement = $parallax = $parallax_image = $css = $el_id = $video_bg = $video_bg_url = $video_bg_parallax = $css_animation = $pgscore_background_position = $overlay_html = $element_css_md = $element_css_sm = $element_css_xs = '';
$disable_element = '';
$output = $after_output = '';
...
if( !empty($pgscore_half_overlap) && $pgscore_half_overlap == 'true' ){
...
<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped ?>>
	<?php
	if( ( isset($pgscore_enable_overlay) && $pgscore_enable_overlay == 'true' ) && (isset($pgscore_overlay_color) && $pgscore_overlay_color != '' ) ){
		?>
...
<?php
if( $after_output != '' ){
	echo wp_kses( $after_output, array(
		'div' => array(
			'class' => array(),
		),
	) );
