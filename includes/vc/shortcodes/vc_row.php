...
function ciyashop_extend_vc_row(){
	...
	$replacing_params = array(
		...
		'el_id' => array(
			...
			'param_name' => 'el_id',
			'description'=> sprintf( esc_html__( 'Enter row ID (Note: make sure it is unique and valid according to <a href="%s" target="_blank">w3c specification</a>)', 'ciyashop' ),
	...
	global $mpc_paths;
	if( $mpc_paths ) {
		$overlay_options = $half_overlap_options = array();
	}
