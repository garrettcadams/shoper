	...
	class Envato_Theme_Setup_Wizard {
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard', 'ciyashop' ), esc_html__( 'Setup Wizard', 'ciyashop' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard', 'ciyashop' ), esc_html__( 'Setup Wizard', 'ciyashop' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
		...
		public function init_wizard_steps() {
			...
			$this->steps = apply_filters( 'envato_theme_setup_wizard_steps', $this->steps );
		...
		public function setup_wizard() {
			...
			wp_register_script( 'match-height', get_parent_theme_file_uri('/js/matchheight/jquery.matchHeight-min.js'), array('jquery'), '', true );
				...
				if ( count( $plugins['all'] ) ) {
					...
					<ul class="envato-wizard-plugins">
						...
						if ( ! empty( $recommended ) ){
							?>
							<li class="plugins-title"><?php esc_html_e( 'Optional', 'ciyashop' ); ?></li>
							<?php $this->_list_plugins( $recommended, $plugins, false, 'recommended' );
						}
						?>
					</ul>
					<?php
		...
		private function _plugin_list_item( $slug, $plugin, $plugins, $checked = false, $plugin_type ) {
			...
			<li data-slug="<?php echo esc_attr( $slug ); ?>" class="plugin-to-install">
				<label for="plugin-import[<?php echo esc_attr($slug);?>]">
					<?php
					echo sprintf( '<input type="checkbox" name="%s" id="%s"%s%s>',
						"plugin-import[$slug]",
						"plugin-import[$slug]",
						checked( ( (isset($plugin['checked_in_wizard']) && $plugin['checked_in_wizard'] != '') || $checked ), true, false ),
						disabled( ( (isset($plugin['checked_in_wizard']) && $plugin['checked_in_wizard'] != '') || $checked ), true, false )
					);
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'ciyashop' ) ) );
			}
			$json = array();
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'ciyashop' ) ) );
			}
			exit;
		...
		public function envato_setup_default_content() {
			...
			<form method="post">
						...
						foreach ($sample_datas as $sample_data_k => $sample_data) {
							...
							<div class="<?php echo esc_attr($sample_data_classes);?>" data-version="<?php echo esc_attr($sample_data_k);?>">
								<div class="sample-content-view">
									<?php
									if( file_exists($sample_data['data_dir'].'preview.jpg') ){
										?>
										<div class="sample-content-thumb">
											<img src="<?php echo esc_url($sample_data['data_url']);?>/preview.jpg" alt="<?php echo esc_attr($sample_data['name']);?>"/>
										</div>
										<?php
									}else{
										?>
				...
				<input type="hidden" name="sample_data_security" id="sample_data_security" value="<?php echo esc_attr(wp_create_nonce( "sample_data_security"));?>">
		...
		private function _make_child_theme( $child_theme_title ) {
			...
			if ( !file_exists($child_theme_path) || ( file_exists($child_theme_path) && $force_child_creation ) ) {
				...
				foreach( $child_theme_template_files as $child_theme_template_file => $child_theme_template_file_type ){
					...
					if( $child_theme_template_file_type == 'template' ){
