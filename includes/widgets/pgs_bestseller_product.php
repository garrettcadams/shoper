 ...
 * @package CiyaShop
 */
class PGS_bestseller_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'PGS_bestseller_Widget', // Base ID
			esc_html__( 'PGS Bestseller Products', 'ciyashop' ), // Name
			array( 'description' => esc_html__( 'A Bestseller Products Sidebar Widget', 'ciyashop' ) ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		// Query args
		$query_args = apply_filters( 'pgs_bestseller_products_widget_args', array(
			'post_type'          => 'product',
			'post_status'        => 'publish',
							...
							while ( $products->have_posts() ) {
								...
								<div class="sellers-row clearfix">
									...
									<div class="item-detail">
										<h4><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute();?>" class="bestseller-product-title"><?php the_title(); ?></a></h4>
	...
	public function form( $instance ) {
		$title       = ! empty( $instance['title'] )       ? $instance['title']       : esc_html__( 'Bestseller Products', 'ciyashop' );
